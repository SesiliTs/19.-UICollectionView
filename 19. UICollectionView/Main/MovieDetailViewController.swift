...
//  17. UITableView
//
//  Created by Sesili Tsikaridze on 04.11.23.
//
...
class MovieDetailViewController: UIViewController {
    ...
    private let filmPoster = {
        ...
        // აქ aspectfill-ით ან სხვა შემთხვევებშიც სხვა ვიუებზე გადადიოდა ფოტო და ვერაფრით ამოვხსენი რატო
    ...
    private func setupUI() {
        filmPoster.image = selectedItem?.image
        filmNameLabel.text = selectedItem?.name
        ratingLabel.text = "\(selectedItem?.rating ?? 0)"
        ...
        releaseValueLabel.text = "2022"
        genreValueLabel.text = "\(selectedItem?.category ?? .Action)"
